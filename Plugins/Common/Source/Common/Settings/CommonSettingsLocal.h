...
public:
	...
	static UCommonSettingsLocal* Get();
...
public:
	...
	ECommonStatDisplayMode GetPerfStatDisplayState(ECommonDisplayablePerformanceStat Stat) const;
	
	/** Sets the display mode for the specified performance stat */
	void SetPerfStatDisplayState(ECommonDisplayablePerformanceStat Stat, ECommonStatDisplayMode DisplayMode);

	/** Fired when the display state for a performance stat has changed, or the settings are applied */
	DECLARE_EVENT(UCommonSettingsLocal, FPerfStatSettingsChanged);
...
private:
	...
	UPROPERTY(Config)
	TMap<ECommonDisplayablePerformanceStat, ECommonStatDisplayMode> DisplayStatList;
...
private:
	...
	void OverrideQualityLevelsToScalabilityMode(const FCommonScalabilitySnapshot& InMode, Scalability::FQualityLevels& InOutLevels);
...
private:
	...
	FCommonScalabilitySnapshot DeviceDefaultScalabilitySettings;
...
public:
	...
	void AddOrUpdateCustomKeyboardBindings(const FName MappingName, const FKey NewKey, UCommonLocalPlayer* LocalPlayer);
	...
	void ResetKeybindingToDefault(const FName MappingName, UCommonLocalPlayer* LocalPlayer);
	...
	void ResetKeybindingsToDefault(UCommonLocalPlayer* LocalPlayer);
