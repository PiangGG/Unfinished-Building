...
void UDialogueGraphSchema::GetGraphContextActions(FGraphContextMenuBuilder& ContextMenuBuilder) const
{
	...
	//NPC nodes can attach to another NPC node, or a player node
	if (bIsNPC)
	{
		FCategorizedGraphActionListBuilder NPCActionBuilder(TEXT("Next we should..."));

		TSharedPtr<FDialogueSchemaAction_NewNode> AddNPC = UDialogueGraphSchema::AddNewNodeAction(NPCActionBuilder, FText::GetEmpty(), FText::FromString("Add another NPC response..."), FText::GetEmpty());
		...
		TSharedPtr<FDialogueSchemaAction_NewNode> AddPlayer = UDialogueGraphSchema::AddNewNodeAction(NPCActionBuilder, FText::GetEmpty(), FText::FromString("Add player response..."), FText::GetEmpty());
		...
		ContextMenuBuilder.Append(NPCActionBuilder);
	}
	else
	{
		FCategorizedGraphActionListBuilder NPCActionBuilder(TEXT("Next we should..."));
		...
		TSharedPtr<FDialogueSchemaAction_NewNode> AddNPC = UDialogueGraphSchema::AddNewNodeAction(NPCActionBuilder, FText::GetEmpty(), FText::FromString("Add another NPC response..."), FText::GetEmpty());
		UDialogueGraphNode* Node = NewObject<UDialogueGraphNode>(ContextMenuBuilder.OwnerOfTemporaries, UDialogueGraphNode_NPC::StaticClass());
		AddNPC->NodeTemplate = Node;
...
UEdGraphNode* FDialogueSchemaAction_AddComment::PerformAction(class UEdGraph* ParentGraph, UEdGraphPin* FromPin, const FVector2D Location, bool bSelectNewNode /*= true*/)
{
	...
	if (UDialogueAsset* const DialogueAsset = Cast<UDialogueAsset>(ParentGraph->GetOuter()))
	{
		if (UAssetEditorSubsystem* AESubsystem = GEditor->GetEditorSubsystem<UAssetEditorSubsystem>())
		{
			DialogueEditor = static_cast<FDialogueGraphEditor*>(AESubsystem->FindEditorForAsset(DialogueAsset, false));
		}
	}

	FSlateRect Bounds;
	if (DialogueEditor && DialogueEditor->GetBoundsForSelectedNodes(Bounds, 50.0f))
	{
